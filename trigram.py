...
class Perceptron:
    ...
    def get_feature(self, sent, idx, label):
        ...
        if (idx - 1 >= 0 and idx + 1 < len(sent)):
            ...
            features.append(sent[idx-1][0] + char + '_' + label + sent[idx+1][0])
        vec = []
        for f in features:
...
def main():
    ...
    #classifier.extract('tri_feature_set')
    #classifier.train('tri_feature_set', 'weight.tri.full')
    classifier.predict('data/test.txt', 'tri_feature_set', 'weight.tri.full')
